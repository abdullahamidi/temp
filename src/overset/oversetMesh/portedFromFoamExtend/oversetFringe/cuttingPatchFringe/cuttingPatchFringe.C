...
void Foam::cuttingPatchFringe::calcAddressing() const
{
    ...
    const labelUList& owner = mesh.faceOwner(); //const unallocLabelList& owner = mesh.faceOwner();
    const labelUList& neighbour = mesh.faceNeighbour(); //const unallocLabelList& neighbour = mesh.faceNeighbour();
    ...
    DynamicList<label> fringeHoles(myRegionCells.size()/10);
    ...
    fringeHolesPtr_ = new labelList(std::move(fringeHoles));
