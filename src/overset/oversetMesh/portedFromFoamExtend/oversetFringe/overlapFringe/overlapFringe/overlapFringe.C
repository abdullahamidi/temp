...
)
{
    ...
    )
    {
        forAll(psib, patchI)
        {
            ...
            {
                psip.initEvaluate(Pstream::defaultCommsType);
            }
        }
        ...
        forAll(psib, patchI)
        {
            ...
            if (psip.coupled() && !isA<oversetFvPatchScalarField>(psip))
            {
                psip.evaluate(Pstream::defaultCommsType);
            }
        }
    }
    else if (Pstream::defaultCommsType == Pstream::commsTypes::scheduled)
    {
        ...
        const lduSchedule& patchSchedule =
            psib[0].internalField().mesh().globalData().
            patchSchedule();
        ...
        forAll(patchSchedule, patchEvalI)
        {
            if (patchSchedule[patchEvalI].init)
            {
                ...
                if (psip.coupled() && !isA<oversetFvPatchScalarField>(psip))
                {
                    psip.initEvaluate(Pstream::commsTypes::scheduled);
                }
            }
            else
            {
                ...
                if (psip.coupled() && !isA<oversetFvPatchScalarField>(psip))
                {
                    psip.evaluate(Pstream::commsTypes::scheduled);
                }
            }
        }
    }
...
void Foam::overlapFringe::calcAddressing() const
{
    ...
    scalarField& processorIndicatorIn = const_cast<scalarField&>(processorIndicator.internalField().field());
    ...
    // mask
    DynamicList<label> candidateAcceptors(mesh.nCells());
    ...
    // Get boundary field
    volScalarField::Boundary& processorIndicatorBf =
        const_cast<volScalarField::Boundary&>(processorIndicator.boundaryField());
    ...
    forAll (processorIndicatorBf, patchI)
    {
        ...
        {
            // Get neighbour field
            const scalarField nbrProcIndicator(chipf.patchNeighbourField());
    ...
    acceptorsPtr_ = new labelList(std::move(candidateAcceptors));
    fringeHolesPtr_ = new labelList(std::move(allHoles.toc()));
}
...
) const
{
    ...
    if (suitabilityFrac > minGlobalFraction_)
    {
        ...
        scalarField& holeIndicatorIn = const_cast<scalarField&>(holeIndicator.internalField().field());

        // Transfer fringeHolesPtr into the dynamic list for efficiency. Note:
        // will be transfered back at the end of the scope.
        DynamicList<label> allFringeHoles(std::move(*fringeHolesPtr_));
        ...
        // Get boundary field
        volScalarField::Boundary& holeIndicatorb =
            const_cast<volScalarField::Boundary&>(holeIndicator.boundaryField());
    ...
    else
    {
        ...
        // Transfer fringeHolesPtr into the dynamic list for efficiency. Note:
        // will be transfered back at the end of the scope.
        DynamicList<label> cumFringeHoles(std::move(*fringeHolesPtr_));
        ...
        DynamicList<label> newAcceptors(10*unsuitableDAPairs.size());
