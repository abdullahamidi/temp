...
) const
{
    ...
    (
        iterationData
        (
            std::move(storageDAPairs),
            std::move(allFringeHoles),
    ...
    if (adaptiveOverlapFringe::debug)
    {
        ...
        scalarField& acceptorsIn = const_cast<scalarField&>(storedAcc.internalField().field());
        ...
        scalarField& holesIn = const_cast<scalarField&>(holes.internalField().field());
    ...
    )
    {
        ...
        DynamicList<label> newAcceptors(2*donorAcceptorRegionData.size());
    ...
    else
    {
        ...
        // Get fringe holes
        DynamicList<label> fringeHoles(maxObject().fringeHoles());
