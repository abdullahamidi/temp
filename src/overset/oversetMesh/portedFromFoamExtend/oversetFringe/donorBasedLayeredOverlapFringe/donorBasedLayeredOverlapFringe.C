...
const Foam::scalar 
Foam::donorBasedLayeredOverlapFringe::distTol_ = 0;
...
void Foam::donorBasedLayeredOverlapFringe::init() const
{
    ...
    forAll (connectedRegionNames_, crI)
    {
        ...
        const label regionID = allRegionNames.find(crName); //findIndex(allRegionNames, crName);
        ...
        forAll (connectedRegionIDs_, crI)
        {
            ...
            for (label i = 0; i < nLayers_; ++i)
            {
                ...
                (
                    mesh,
                    propagateFace,
                    orEqOp<bool>() //orOp<bool>() // S.A {13.09.22}
                );
