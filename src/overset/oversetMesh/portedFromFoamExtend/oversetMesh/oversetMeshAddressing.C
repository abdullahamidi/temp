...
void Foam::oversetMesh::calcDomainMarkup() const
{
    ...
    scalarField& overTypesIn = const_cast<scalarField&>(overTypes.internalField().field());
    ...
    forAll (overTypes.boundaryField(), patchI)
    {
        ...
        {
            overTypes.boundaryFieldRef()[patchI] =
    ...
    scalarField& regionIDIn = const_cast<scalarField&>(regionID.internalField().field());
    ...
    volScalarField::Boundary& regionIDb =
        const_cast<volScalarField::Boundary&>(regionID.boundaryField());
...
void Foam::oversetMesh::calcGamma() const
{
    ...
    scalarField& gIn = const_cast<scalarField&>(g.internalField().field());
    ...
    scalarField& gExtIn = const_cast<scalarField&>(gExt.internalField().field());
    ...
    scalarField& sgIn = const_cast<scalarField&>(sg.internalField().field());
    ...
    gAcc.boundaryFieldRef().evaluate();
    ...
    surfaceScalarField::Boundary sgPatches =
        sg.boundaryField();

    const volScalarField::Boundary& gExtPatches =
        gExt.boundaryField();

    const volScalarField::Boundary& gAccPatches =
        gAcc.boundaryField();
    ...
    forAll (gExtPatches, patchI)
    {
        if (gExtPatches[patchI].coupled())
        {
            ...
            const scalarField gammaOwn(gExtPatches[patchI].patchInternalField());

            const scalarField gammaNei(gExtPatches[patchI].patchNeighbourField());
            ...
            const scalarField gammaAccOwn(gAccPatches[patchI].patchInternalField());

            const scalarField gammaAccNei(gAccPatches[patchI].patchNeighbourField());
        ...
        else
        {
            ...
            // HJ, 21/May/2012
            const scalarField gammaFc(g.boundaryField()[patchI].patchInternalField());
...
void Foam::oversetMesh::calcFringeFaces() const
{
    ...
    forAll (gammaPatches, patchI)
    {
        ...
        {
            const scalarField gammaOwn(gammaPatches[patchI].patchInternalField());

            const scalarField gammaNei(gammaPatches[patchI].patchNeighbourField());
    ...
    fringeFacesPtr_ = new labelList(std::move(acF));

    fringeFaceCellsPtr_ = new labelList(std::move(acFC));
    fringeFaceFlipsPtr_ = new boolList(std::move(acFF));
    ...
    const volScalarField::Boundary& gammaExtPatches =
    ...
    forAll (gammaPatches, patchI)
    {
        ...
        if (gammaPatches[patchI].coupled())
        {
            ...
            const scalarField gammaAccOwn(gammaExtPatches[patchI].patchInternalField()
              - gammaPatches[patchI].patchInternalField());

            const scalarField gammaAccNei(gammaExtPatches[patchI].patchNeighbourField()
    ...
    acceptorInternalFacesPtr_ = new labelList(std::move(acInternalF));
}
...
void Foam::oversetMesh::calcHoleFaces() const
{
    ...
    forAll (gammaExtPatches, patchI)
    {
        ...
        {
            const scalarField gammaExtOwn(gammaExtPatches[patchI].patchInternalField());

            const scalarField gammaExtNei(gammaExtPatches[patchI].patchNeighbourField());
    ...
    holeFacesPtr_ = new labelList(std::move(hF));

    holeFaceCellsPtr_ = new labelList(std::move(hFC));
    holeFaceFlipsPtr_ = new boolList(std::move(hFF));
    ...
    // Pack the data
    hIF.shrink();
    holeInternalFacesPtr_ = new labelList(std::move(hIF));
}
...
void Foam::oversetMesh::calcInterpolationMap() const
{
    ...
    */
    ...
    (
        nReceives,
        std::move(sendDataMap),
        std::move(constructDataMap)
    );
