...
void Foam::oversetRegion::calcCellSearch() const
{
    ...
    (
        ...
        (
            ...
            mesh_,
            eligibleDonors(),
            polyMesh::CELL_TETS //How to decide if a point is inside cell (CELL_TETS -> Cell decomposed into tets)
        ),
...
bool Foam::oversetRegion::updateDonorAcceptors() const
{
    ...
    List<DynamicList<label>> sendAcceptorMap(Pstream::nProcs());
    ...
    forAll (sendAcceptorMap, procI)
    {
        ...
        DynamicList<label>& curSendMap = sendAcceptorMap[procI];
    ...
    */
    ...
    (
        nAcceptorReceives,
        std::move(sendAcceptorFixedMap),
        std::move(constructAcceptorMap)//,
        //true // reuse maps
    );
    ...
    (
        nDonorReceives,
        std::move(sendDonorMap),
        std::move(constructDonorMap) //,
        //true // reuse maps
    );
...
void Foam::oversetRegion::finaliseDonorAcceptors() const
{
    ...
    List<DynamicList<label>> sendMap(Pstream::nProcs());
...
{
    if (fringePtr_ == nullptr) //if (fringePtr_.empty()) // S.A {13.09.22}
    {
