...
defineTypeNameAndDebug(laxFriedrichsTams, 0);
addToRunTimeSelectionTable(jacobianInviscid, laxFriedrichsTams, dictionary);
// Backward compatibility - misspelling:
//addNamedToRunTimeSelectionTable(jacobianInviscid, laxFriedrichsTams, dictionary, LaxFriedrich);


// * * * * * * * * * * * * * Constructor * * * * * * * * * * * * * * * * * * //

laxFriedrichsTams::laxFriedrichsTams
(
...
{
    //// S.A {Sep. 2022}
    if(mesh_.foundObject<surfaceScalarField>("minusMRFFlux"))
    {
        //Info<<"MRF simulation is detected at laxFriedrichsTams"<<endl;
        MRF = true;
    }
    ////
}
...
// flux terms.
void laxFriedrichsTams::addFluxTerms(compressibleJacobianMatrix& jacobian)
{
#if OPENFOAM >= 1712
    ...
    {
        const surfaceScalarField& minusMRFFlux = mesh_.lookupObject<surfaceScalarField>("minusMRFFlux");
        tmp<fvjMatrix<scalar> > divMRFPhi = fvj::div(w, -minusMRFFlux);

        dContByRho -= divMRFPhi();
        dMomByRhoU -= divMRFPhi()*tensor::I;
        dEnergyByRhoE -= divMRFPhi;
    }              ////
...
void laxFriedrichsTams::boundaryJacobian
(
    label patchi,
...
)
{
    ...
    {
        const surfaceScalarField& minusMRFFlux = mesh_.lookupObject<surfaceScalarField>("minusMRFFlux");
        UrelBdotSf.ref() += minusMRFFlux.boundaryField()[patchi];
    }              ////
...
void laxFriedrichsTams::addInviscidJacobian(compressibleJacobianMatrix& jacobian)
{
    ...
    {
        const surfaceScalarField& minusMRFFlux = mesh_.lookupObject<surfaceScalarField>("minusMRFFlux");
        lambdaConv_ = (fvc::interpolate(c) + mag((fvc::interpolate(U)&mesh_.Sf())+minusMRFFlux)/mesh_.magSf())/mesh_.deltaCoeffs();
    }              ////
    else
    {
