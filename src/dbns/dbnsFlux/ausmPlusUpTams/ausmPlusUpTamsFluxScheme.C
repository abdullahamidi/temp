...
#include "ausmPlusUpTamsFluxScheme.H"
...
defineTypeNameAndDebug(ausmPlusUpTamsFluxScheme, 0);
addToRunTimeSelectionTable(fluxScheme, ausmPlusUpTamsFluxScheme, dictionary);
...
ausmPlusUpTamsFluxScheme::ausmPlusUpTamsFluxScheme
(
...
void Foam::ausmPlusUpTamsFluxScheme::calcFlux(surfaceScalarField& phi, surfaceVectorField& phiUp, surfaceScalarField& phiEp, surfaceVectorField& Up)
{
    bool lowMach =
    ...
    //// S.A {Sep. 2022}
    bool MRF = false;
    autoPtr<surfaceScalarField> minusMRFFlux_;
    if(mesh_.foundObject<surfaceScalarField>("minusMRFFlux"))
    {
        //Info<<"MRF simulation is detected at AUSMplusUP"<<endl;
        MRF = true;
        ...
        (
            ...
            (
                ...
                mesh_.lookupObject<surfaceScalarField>("minusMRFFlux")
            )
        );
    }
    ////
    ...
    {
        phi_L.ref() += minusMRFFlux_();
        phi_R.ref() += minusMRFFlux_();
    }
                  ////
    ...
    {
        Hrel = H() - 0.5*(U_&U_);
        volVectorField Urel(U_ - fvc::reconstruct(-minusMRFFlux_()));
        Hrel.ref() += 0.5*(Urel&Urel);
    }
                  ////
...
//bool transient = false;
    ...
    else if (MRF) //// S.A {Sep. 2022}
    {
        ...
        minusMRFFlux.setOriented(true);
        phiEp -= p_1_2 * minusMRFFlux;
    }
                  ////
}
