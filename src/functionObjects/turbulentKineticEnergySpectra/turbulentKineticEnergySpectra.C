...
defineTypeNameAndDebug(turbulentKineticEnergySpectra, 0);
addToRunTimeSelectionTable(functionObject, turbulentKineticEnergySpectra, dictionary);


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void turbulentKineticEnergySpectra::writeFileHeader(Ostream& os) {
...
turbulentKineticEnergySpectra::turbulentKineticEnergySpectra
(
    const word& name,
    const Time& runTime,
...
data_count_(0) {
    read(dict);
    Info << "turbulentKineticEnergySpectra" << nl;
}


// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

bool turbulentKineticEnergySpectra::read(const dictionary& dict) {
...
bool turbulentKineticEnergySpectra::execute() {
    return true;
}

void turbulentKineticEnergySpectra::read_probe() {
    Info << "Reading " << probeDir_ / "U" << nl;
    vdata_ = readVectorProbe(probeDir_ / "U", begin_time_, delta_t_, data_count_);
}

void turbulentKineticEnergySpectra::do_calculate(const Field<vectorField>& vdata, label data_count, scalar delta_t, label window_count, scalar window_overlap, fileName output) {
    ...
    forAll(u_prime, i) {
        label data_count;
        spectra[i] = single_sided_psd(u_prime[i], delta_t, window_count, window_overlap, data_count);
    }
...
bool turbulentKineticEnergySpectra::write() {
    read_probe();
