...
inline void gmres_rightPrecond<nScalar, nVector>::givensRotation
(
    const scalar& h,
    const scalar& beta,
    scalar& c,
...
template <int nScalar, int nVector>
gmres_rightPrecond<nScalar, nVector>::gmres_rightPrecond
(
...
template <int nScalar, int nVector>
label gmres_rightPrecond<nScalar, nVector>::solve
(
...
) const
{
    ...
    // Switch for Adaptive Krylov Subspace
    bool AKSS = dict.lookupOrDefault<bool>("adaptiveKrylov", true);
    ...
    PtrList<volScalarField> sTmp2(nScalar);
    PtrList<volVectorField> vTmp2(nVector);
    forN(nScalar,j)
    {
        sTmp2.set
        (
    ...
    while(1)                                            // Outer loop
    {
        ...
        //this->precondition(sTmp, vTmp);             // P^-1 v_o  &  P^-1 A \Delta U_0   //Left-Preconditioning
        ...
        bh = 0;
        scalar norm0 = GREAT; //// for Adaptive Krylov Subspace Sizing (AKSS)
        ...
        for (label i = 0; i < nDirs; i++)				// Search directions
        {
            ...
            forN(nScalar,j)
            {
                ...
                sTmp2[j].primitiveFieldRef() = sTmp[j].primitiveField();
            }
            forN(nVector,j)
            {
                vV[j].primitiveFieldRef() = vTmp[j].primitiveField();
                vTmp2[j].primitiveFieldRef() = vTmp[j].primitiveField();
            }

            // Matrix vector product
            //this->jacobian_.matrixMul(sV, vV, sTmp, vTmp);    // y_j = A v_j       //Left-Preconditioning

            // Execute preconditioning
            //this->precondition(sTmp, vTmp);			// w_j = P^-1 y_j    //Left-Preconditioning
            
            // Execute preconditioning
            this->precondition(sTmp2, vTmp2);			//Right-Preconditioning
            
            // Matrix vector product
            this->jacobian_.matrixMul(sTmp2, vTmp2, sTmp, vTmp);      //Right-Preconditioning
...
//// AKSS
            if (i==0 && AKSS)
            {
                norm0 = beta;  //Info<<"initial beta: "<<norm0<<endl;
            }
            //Info<<"updated beta: "<<beta<<endl;
            if(beta<norm0*max(tolRel) && AKSS)
            {
                nDirs = i;
                break;
            }
//// AKSS                      
        ...
        this->precondition(dsW, dvW); //Right-Preconditioning
    ...
    if(this->mesh_.template foundObject<volScalarField>("cellMask"))
    {
        ...
        forN(nScalar,i)
        {
            dsW[i].primitiveFieldRef() *= cellMask;
        }
        forN(nVector,i)
        {
            dvW[i].primitiveFieldRef() *= cellMask;
        }
    }
    ////
