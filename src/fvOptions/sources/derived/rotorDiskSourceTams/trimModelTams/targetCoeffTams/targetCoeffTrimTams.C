...
namespace Foam
{
    defineTypeNameAndDebug(targetCoeffTrimTams, 0);
    addToRunTimeSelectionTable(trimModelTams, targetCoeffTrimTams, dictionary);
}
...
Foam::vector Foam::targetCoeffTrimTams::calcCoeffs
(
...
template<class RhoFieldType>
void Foam::targetCoeffTrimTams::correctTrim
(
    const RhoFieldType& rho,
    const vectorField& U,
    vectorField& force
...
Foam::targetCoeffTrimTams::targetCoeffTrimTams
(
    const fv::rotorDiskSourceTams& rotor,
    const dictionary& dict
)
:
    trimModelTams(rotor, dict, typeName),
    calcFrequency_(-1),
    useCoeffs_(true),
    target_(Zero),
    theta_(Zero),
...
void Foam::targetCoeffTrimTams::read(const dictionary& dict)
{
    trimModelTams::read(dict);
...
Foam::tmp<Foam::scalarField> Foam::targetCoeffTrimTams::thetag() const
{
...
void Foam::targetCoeffTrimTams::correct
(
    const vectorField& U,
    vectorField& force
)
