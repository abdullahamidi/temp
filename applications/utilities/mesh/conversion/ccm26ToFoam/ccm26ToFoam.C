...
#   include "createTime.H"
    ...
    {
        fileName ccmFile = args[1]; //fileName ccmFile(args.additionalArgs()[0]);
    ...
    (
        ...
        std::move(foamPoints), //xferMove<pointField>(foamPoints),
        std::move(foamFaces), //xferMove<faceList>(foamFaces),
        labelList(foamOwner), //xferCopy<labelList>(foamOwner),
        std::move(foamNeighbour) //xferMove<labelList>(foamNeighbour)
    );
