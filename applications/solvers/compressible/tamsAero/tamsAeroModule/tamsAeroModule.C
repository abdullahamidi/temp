...
namespace Foam
{
defineTypeNameAndDebug(tamsAeroModule, 0);
}
...
void tamsAeroModule::createPreconditioners
(
...
void tamsAeroModule::parallelSyncFields(const wordList& fields)
{
    forAll(fields, i)
    {
...
void tamsAeroModule::redistributePar()
{
    fvMesh& mesh = mesh_();
...
void tamsAeroModule::initialise()
{
    ...
    if (overset_){
        cellTypes_ = &objectRegistry::store(new volScalarField(
            IOobject (
                word("cellTypes"),
                runTime.timeName(),
                mesh,
...
scalar tamsAeroModule::timeStepScaling(const scalar& maxCoNum)
{
    ...
    #include "compressibleCourantNoTams.H"
...
void tamsAeroModule::beginTimeStep()
{
...
void tamsAeroModule::outerIteration()
{
    ...
    {
        volScalarField rhoMin(localTimesteppingLowerBound_*scalarVarsPrevIter_[0]);
        ...
        forAll(mesh.boundary(), patchi)
        {
            ...
            {
                scalarField rhoNei(rho.boundaryField()[patchi].patchNeighbourField());
                scalarField rhoMinNei(rhoMin.boundaryField()[patchi].patchNeighbourField());
                scalarField rhoMaxNei(rhoMax.boundaryField()[patchi].patchNeighbourField());
                scalarField eTempNei(eTemp.boundaryField()[patchi].patchNeighbourField());
                scalarField eMinNei(eMin.boundaryField()[patchi].patchNeighbourField());
                scalarField eMaxNei(eMax.boundaryField()[patchi].patchNeighbourField());
...
void tamsAeroModule::findDebugCell()
{
